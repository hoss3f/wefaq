...
- `onboarding.steps[]`: ordered array of question definitions, each with `key`, `type` (`gender` / `text` / `date` / `search` / `choice` / `chips` / `number` / `textarea` / `preferences` / `contact`), `title`, `required`, optional `options`, optional `storage: "personal"` (writes directly to a `User` column — `full_name`, `birthday`, `gender`, `country`, `phone`, `email`) or otherwise stored under `UserProfile.details[key]`
- The `contact` step type (placed right after the `full_name` step) asks for phone + email on one screen: a country-dial-code `<select>` (from the step's `country_codes[]`, e.g. `{code:"+974", name:"قطر"}`) next to the phone number input, plus a separate email input below. Rendered by a dedicated branch in `CompleteApplicationPage.jsx`'s `renderQuestion()` (not the generic `getValue`/`setValue` path, since it writes two `personal` keys — `phone` combines the selected dial code + digits into one string, `email` is written directly); phone is required, email is optional (nullable on `User`)
...
- Email: `send_welcome_email(user)` — sends an Arabic/RTL HTML welcome email (via `smtplib`, SMTP creds from env: `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `SMTP_FROM`) after `POST /users/<id>/complete` succeeds; no-ops silently if `user.email` is empty or SMTP env vars are missing, and swallows/logs send failures so application submission always succeeds regardless of email delivery. See `.env.example`.
...
- `POST /users/<id>/complete`: **primary first-login completion**, used by the dynamic wizard. Body `{ personal: {...+profile_details}, mcq, open }`. Validates personal fields + `profile_details` (nationality/profession/marital_status/marriage_timeline/height/weight) via `_apply_personal_data` (also sanitizes `phone` and validates `email` format if provided — both optional/nullable), requires all 4 open answers, upserts `MCQAnswer`/`OpenAnswer`, sets `reviewing`, syncs JSON, fires `send_welcome_email(user)` (no-op if `user.email` empty or SMTP unset — never fails the request), returns user + `mcq_answers` + `profile_details` + `open_answers` with `needs_onboarding: false`.
